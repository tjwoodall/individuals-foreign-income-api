...
@Singleton
class DeleteForeignValidatorFactory @Inject() (implicit foreignIncomeConfig: ForeignIncomeConfig) {
  private lazy val minTaxYear = foreignIncomeConfig.minimumPermittedTaxYear()

  def validator(nino: String, taxYear: String): Validator[DeleteForeignRequest] = new Validator[DeleteForeignRequest] {

    def validate: Validated[Seq[MtdError], DeleteForeignRequest] =
      (
        ResolveNino(nino),
