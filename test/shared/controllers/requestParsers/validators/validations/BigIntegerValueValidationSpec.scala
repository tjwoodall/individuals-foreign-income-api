    ...
    "validate" must {
      ...
      "return a ValueFormatError for an invalid integer value (default validation)" in {
        BigIntegerValueValidation.validate(
          field = -2,
          path = "/path"
        ) shouldBe List(ValueFormatError.copy(message = ZERO_MINIMUM_BIG_INTEGER_INCLUSIVE, paths = Some(List("/path"))))
      }
      ...
      "return a ValueFormatError for an invalid integer value (custom validation)" in {
        ...
        ) shouldBe List(ValueFormatError.copy(message = "error message", paths = Some(List("/path"))))
      }
    }
