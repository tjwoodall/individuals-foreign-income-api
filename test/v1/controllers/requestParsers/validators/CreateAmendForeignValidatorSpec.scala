...
class CreateAmendForeignValidatorSpec extends UnitSpec with ValueFormatErrorMessages {
  ...
  "running a validation" should {
    "return no errors" when {
      "a valid request is supplied" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, validRawRequestBody)) shouldBe Nil
      }
    }

    "return NinoFormatError error" when {
      "an invalid nino is supplied" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData("A12344A", validTaxYear, validRawRequestBody)) shouldBe
    ...
    "return TaxYearFormatError error" when {
      "an invalid tax year is supplied" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, "20178", validRawRequestBody)) shouldBe
    ...
    "return RuleTaxYearNotSupported error" when {
      "an invalid tax year is supplied" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, "2017-18", validRawRequestBody)) shouldBe
    ...
    "return RuleIncorrectOrEmptyBodyError error" when {
      "an empty JSON body is submitted" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, emptyRawRequestBody)) shouldBe
          List(RuleIncorrectOrEmptyBodyError)
      }

      "a non-empty JSON body is submitted without any expected fields" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, nonsenseRawRequestBody)) shouldBe
          List(RuleIncorrectOrEmptyBodyError)
      }

      "the submitted request body is not in the correct format" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, nonValidRawRequestBody)) shouldBe
          List(
      ...
      "the submitted request body has missing mandatory fields" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, missingMandatoryFieldRequestBody)) shouldBe
          List(
            RuleIncorrectOrEmptyBodyError.copy(paths = Some(
              List(
    ...
    "return CustomerRefFormatError error" when {
      "an incorrectly formatted customer reference is submitted" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, invalidCustomerReferenceRequestBody)) shouldBe
    ...
    "return ValueFormatError error" when {
      "an incorrectly formatted earningsNotTaxableUK is submitted" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, invalidEarningsNotTaxableUKRequestBody)) shouldBe
          List(
    ...
    "return ValueFormatError error (single failure)" when {
      "one field fails value validation (countryCode 3 digit)" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, invalidCountryCodeRequestBody)) shouldBe
          List(
      ...
      "one field fails value validation (countryCode 4 digit)" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, invalidCountryCodeFormatRequestBody)) shouldBe
          List(
      ...
      "one field fails value validation (amountInForeignCurrency)" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, invalidAmountInForeignCurrencyRequestBody)) shouldBe
          List(
            ValueFormatError.copy(
              message = ZERO_MINIMUM_INCLUSIVE,
      ...
      "one field fails value validation (AmountTaxPaid)" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, invalidAmountTaxPaidRequestBody)) shouldBe
          List(
            ValueFormatError.copy(
              message = ZERO_MINIMUM_INCLUSIVE,
    ...
    "return ValueFormatError error (multiple failures)" when {
      "multiple fields fail value validation" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData(validNino, validTaxYear, allInvalidValueRawRequestBody)) shouldBe
          List(
            CountryCodeFormatError.copy(
              paths = Some(List("/unremittableForeignIncome/0/countryCode"))
            ),
    ...
    "return multiple errors" when {
      "request supplied has multiple errors (path parameters)" in new Test {
        validator.validate(createAmend.CreateAmendForeignRawData("A12344A", "20178", emptyRawRequestBody)) shouldBe
